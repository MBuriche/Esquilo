...
if (isset($_GET['delete_user']) && is_numeric($_GET['delete_user'])) {
    ...
    $sql = "DELETE FROM usuarios WHERE id = ? AND afiliado = 1";
    $stmt = $mysqli->prepare($sql);
    $stmt->bind_param("i", $user_id);
    
    if ($stmt->execute()) {
        $success_message = "Afiliado excluído com sucesso!";
    } else {
        $error_message = "Erro ao excluir afiliado: " . $mysqli->error;
    }
    $stmt->close();
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['bulk_delete']) && isset($_POST['selected_users'])) {
    ...
    if (!empty($selected_users) && is_array($selected_users)) {
        ...
        $sql = "DELETE FROM usuarios WHERE id IN ($placeholders) AND afiliado = 1";
        $stmt = $mysqli->prepare($sql);
        ...
        if ($stmt->execute()) {
            ...
            $success_message = "Foram excluídos $deleted_count afiliado(s) com sucesso!";
        } else {
            $error_message = "Erro ao excluir afiliados: " . $mysqli->error;
        }
        $stmt->close();
    } else {
        $error_message = "Nenhum afiliado foi selecionado para exclusão.";
    }
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['add_user'])) {
    ...
    if (empty($errors)) {
        $check_sql = "SELECT id FROM usuarios WHERE (email = ? OR usuario = ?) AND afiliado = 1";
    ...
    if (empty($errors)) {
        ...
        $sql = "INSERT INTO usuarios (email, usuario, celular, password, saldo, url, codigo_convite, token, data_registro, cpf, afiliado) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW(), ?, 1)";
        ...
        if ($stmt->execute()) {
            $success_message = "Afiliado adicionado com sucesso!";
        } else {
            $error_message = "Erro ao adicionar afiliado: " . $mysqli->error;
        }
        $stmt->close();
    } else {
...
$where_clause = "WHERE afiliado = 1";
$params = [];
$types = "";

if (!empty($search)) {
    if (is_numeric($search)) {
        $where_clause = "WHERE afiliado = 1 AND (id = ? OR usuario LIKE ? OR email LIKE ? OR celular LIKE ? OR cpf LIKE ?)";
        $params = [$search, "%{$search}%", "%{$search}%", "%{$search}%", "%{$search}%"];
        $types = "issss";
    } else {
        $where_clause = "WHERE afiliado = 1 AND (usuario LIKE ? OR email LIKE ? OR celular LIKE ? OR cpf LIKE ?)";
        $search_param = "%{$search}%";
            ...
            <div class="h-[calc(100vh-73px)] overflow-y-auto overflow-x-hidden">
                <div class="p-7 min-h-[calc(100vh-145px)]">
                    <div class="px-2 py-1 mb-4">
                        <p class="text-sm font-semibold">Gerenciamento de Afiliados</p>
                        <p class="text-xs text-gray-500">Total de afiliados: <?php echo $total_records; ?></p>
                    </div>
