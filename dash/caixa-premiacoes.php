<?php
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_caixa'])) {
    ...
    try {
        ...
        $res = $mysqli->query("SELECT id FROM caixa_premiacoes_status WHERE id=1");
        if (!$res || $res->num_rows === 0) {
            // Tentar inserir com INSERT IGNORE para evitar erro se já existir
            $sql = $mysqli->prepare("INSERT IGNORE INTO caixa_premiacoes_status (id, ativo, valor_atual, porcentagem_distribuicao) VALUES (1, 1, 0.00, 0.00)");
            $sql->execute();
            
            // Verificar se a inserção foi bem-sucedida
            if ($sql->affected_rows === 0) {
                // Se não inseriu, tentar UPDATE para garantir que existe
                $sql = $mysqli->prepare("UPDATE caixa_premiacoes_status SET ativo=1, valor_atual=0.00, porcentagem_distribuicao=0.00 WHERE id=1");
                $sql->execute();
            }
        }
        
        // Ativar/desativar
        if (isset($_POST['caixa_ativo_changed'])) {
            $ativo = isset($_POST['caixa_ativo']) ? 1 : 0;
            
            // Verificar se o registro existe antes de atualizar
            $check = $mysqli->query("SELECT id FROM caixa_premiacoes_status WHERE id=1");
            if (!$check || $check->num_rows === 0) {
                // Criar registro se não existir
                $sql = $mysqli->prepare("INSERT INTO caixa_premiacoes_status (id, ativo, valor_atual, porcentagem_distribuicao) VALUES (1, ?, 0.00, 0.00)");
                $sql->bind_param("i", $ativo);
                $sql->execute();
            } else {
                // Atualizar registro existente
                $sql = $mysqli->prepare("UPDATE caixa_premiacoes_status SET ativo=? WHERE id=1");
                $sql->bind_param("i", $ativo);
                $sql->execute();
            }
            ...
            try {
                $sql2 = $mysqli->prepare("INSERT INTO caixa_premiacoes_log (admin_id, acao) VALUES (?, ?)");
                $sql2->bind_param("is", $admin_id, $acao);
                $sql2->execute();
            } catch (Exception $logError) {
                // Se der erro no log, não falha a operação principal
                error_log("Erro ao inserir log: " . $logError->getMessage());
            }
        }
        // Adicionar valor
        $valor_add = str_replace(['.', ','], ['', '.'], $_POST['valor_adicionar'] ?? '');
        if ($valor_add !== '' && is_numeric($valor_add) && floatval($valor_add) > 0) {
            ...
            if (!$check || $check->num_rows === 0) {
                // Criar registro se não existir
                $sql = $mysqli->prepare("INSERT INTO caixa_premiacoes_status (id, ativo, valor_atual, porcentagem_distribuicao) VALUES (1, 1, ?, 0.00)");
                $sql->bind_param("d", $valor_add);
                $sql->execute();
            } else {
                // Atualizar registro existente
                $sql = $mysqli->prepare("UPDATE caixa_premiacoes_status SET valor_atual=valor_atual+? WHERE id=1");
                $sql->bind_param("d", $valor_add);
                $sql->execute();
            }
            
            try {
                $sql2 = $mysqli->prepare("INSERT INTO caixa_premiacoes_log (admin_id, acao, valor) VALUES (?, 'adicionar_valor', ?)");
                $sql2->bind_param("id", $admin_id, $valor_add);
                $sql2->execute();
            } catch (Exception $logError) {
                // Se der erro no log, não falha a operação principal
                error_log("Erro ao inserir log: " . $logError->getMessage());
        ...
        // Porcentagem
        if (isset($_POST['porcentagem_changed'])) {
            $porcentagem = str_replace(',', '.', $_POST['porcentagem'] ?? '');
            if ($porcentagem !== '' && is_numeric($porcentagem)) {
                ...
                if (!$check || $check->num_rows === 0) {
                    // Criar registro se não existir
                    $sql = $mysqli->prepare("INSERT INTO caixa_premiacoes_status (id, ativo, valor_atual, porcentagem_distribuicao) VALUES (1, 1, 0.00, ?)");
                    $sql->bind_param("d", $porcentagem);
                    $sql->execute();
                } else {
                    // Atualizar registro existente
                    $sql = $mysqli->prepare("UPDATE caixa_premiacoes_status SET porcentagem_distribuicao=? WHERE id=1");
                    $sql->bind_param("d", $porcentagem);
                    $sql->execute();
                }
                
                try {
                    $sql2 = $mysqli->prepare("INSERT INTO caixa_premiacoes_log (admin_id, acao, porcentagem) VALUES (?, 'definir_porcentagem', ?)");
                    $sql2->bind_param("id", $admin_id, $porcentagem);
                    $sql2->execute();
                } catch (Exception $logError) {
                    // Se der erro no log, não falha a operação principal
                    error_log("Erro ao inserir log: " . $logError->getMessage());
        ...
        $status = [];
        $res = $mysqli->query("SELECT * FROM caixa_premiacoes_status WHERE id=1");
...
<body x-data="main"
        ...
        <div class="main-content flex-1">
            ...
            <div class="h-[calc(100vh-73px)] overflow-y-auto overflow-x-hidden">
                ...
                <div class="p-4 sm:p-7 min-h-[calc(100vh-145px)]">
                    <form id="form-caixa-premiacoes" class="space-y-7">
                        <div class="px-2 py-1 mb-4">
                            <h2 class="text-lg font-semibold">Controle De Caixa</h2>
                            <h2 class="text-xs font-semibold">Valor Atual Em Caixa: <span id="valor-atual-caixa"
                                    style="color: green">R$<?php echo number_format($status['valor_atual'], 2, ',', '.'); ?></span>
                            </h2>
                        ...
                        <div class="grid grid-cols-1 lg:grid-cols-2 gap-7">
                            <div class="border border-black/10 dark:border-white/10 p-5 rounded-md">
                                <div class="mb-5">
                                    <p class="text-sm font-semibold">Ativar/Desativar Caixa De Premios</p>
                                    <h2 class="text-xs font-italic">Ative o caixa de premios para distribuir premiações,
                                        ou desative para não distribuir.</h2>
                                </div>
                                <div>
                                    <div class="togglebutton inline-block">
                                        <label for="toggleD2" class="flex items-center cursor-pointer">
                                            <div class="relative">
                                                <input type="checkbox" id="toggleD2" name="caixa_ativo" value="1"
                                                    class="sr-only" <?php if ($status['ativo'])
                                                        echo 'checked'; ?>
                                                    onchange="this.form.caixa_ativo_changed.value=1">
                                                <div
                                                    class="block band bg-black/20 dark:bg-white/20 w-[52px] h-7 rounded-full">
                                                </div>
                                                <div
                                                    class="dot absolute left-[1.5px] top-[2px] bg-white dark:bg-black w-6 h-6 rounded-full transition">
                                                </div>
                                            </div>
                                        </label>
                                        <input type="hidden" name="caixa_ativo_changed" value="">
                                    </div>
                                </div>
                            </div>
                            <div class="border border-black/10 dark:border-white/10 p-5 rounded-md">
                                <div class="mb-5">
                                    <p class="text-sm font-semibold">Valor Para Adicionar No Caixa</p>
                                    <h2 class="text-xs font-italic">O valor inserido será adicionado ao caixa para
                                        premiações aos jogadores.</h2>
                                </div>
                                <div>
                                    <label class="mt-1 flex -space-x-px">
                                        <div
                                            class="flex items-center justify-center rounded-l-lg border border-black/10 dark:border-white/10 px-3.5">
                                            <span>R$</span>
                                        </div>
                                        <input name="valor_adicionar"
                                            ...
                                            placeholder="Digite O Valor" type="text" />
                                        <div
                                            class="flex items-center justify-center rounded-r-lg border border-black/10 dark:border-white/10 px-3.5">
                                            <span>.00</span>
                                        </div>
                                    </label>
                                </div>
                            ...
                            <div class="border border-black/10 dark:border-white/10 p-5 rounded-md">
                                <div class="mb-5">
                                    <p class="text-sm font-semibold">Distribuição do caixa</p>
                                    <h2 class="text-xs font-italic">A porcentagem inserida será distribuída do valor do
                                        caixa entre os jogadores.</h2>
                                </div>
                                <div>
                                    <label class="mt-1 flex -space-x-px">
                                        <input name="porcentagem" id="porcentagem-caixa"
                                            class="form-input w-full rounded-l-lg border border-black/10 dark:border-white/10 bg-transparent px-3 py-2.5 placeholder:text-black/60  dark:placeholder:text-white/60 hover:z-10 hover:border-black dark:hover:border-white focus:z-10 focus:border-black dark:focus:border-white"
                                            placeholder="Digite A Porcentagem" type="text"
                                            value="<?php echo $status['porcentagem_distribuicao']; ?>"
                                            onchange="this.form.porcentagem_changed.value=1" />
                                        <div
                                            class="flex items-center justify-center rounded-r-lg border border-black/10 dark:border-white/10 px-3.5">
                                            <span>%</span>
                                        </div>
                                    </label>
                                    <input type="hidden" name="porcentagem_changed" value="">
                                </div>
                            </div>
                        </div>
    ...
    <script>
        document.getElementById('form-caixa-premiacoes')?.addEventListener('submit', function (e) {
            e.preventDefault();
            const form = this;
            const fd = new FormData(form);
            fd.append('ajax_caixa', 1);
            fetch('caixa-premiacoes.php', { method: 'POST', body: fd })
                .then(r => r.json())
                .then(data => {
                    showToast(data.message, data.success ? 'success' : 'error');
                    if (data.success && data.status) {
                        // Atualiza valor atual
                        document.getElementById('valor-atual-caixa').textContent = 'R$' + Number(data.status.valor_atual).toLocaleString('pt-BR', { minimumFractionDigits: 2 });
                        // Atualiza status
                        const statusSpan = document.getElementById('status-caixa');
                        statusSpan.textContent = data.status.ativo == 1 ? 'Ativo' : 'Inativo';
                        statusSpan.className = 'font-semibold ' + (data.status.ativo == 1 ? 'text-green-600' : 'text-red-600');
                        // Atualiza porcentagem
                        document.getElementById('porcentagem-caixa').value = data.status.porcentagem_distribuicao;
                        document.getElementById('porcentagem-caixa-label').textContent = Number(data.status.porcentagem_distribuicao).toLocaleString('pt-BR', { minimumFractionDigits: 2 }) + '%';
                    }
                });
        });
    </script>
