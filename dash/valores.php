...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax_valores'])) {
    ...
    try {
        ...
        if ($valores['saque_min'] > $valores['saque_max']) {
            throw new Exception('O saque mínimo não pode ser maior que o saque máximo. (min: ' . $valores['saque_min'] . ', max: ' . $valores['saque_max'] . ')');
        }
        if ($valores['rollover'] < 1) {
            throw new Exception('O rollover deve ser maior ou igual a 1.');
        }
...
<body x-data="main"
        ...
        <div class="main-content flex-1">
            ...
            <div class="h-[calc(100vh-73px)] overflow-y-auto overflow-x-hidden">
                ...
                function formatarReais($valor) {
                    return $valor !== null && $valor !== '' ? number_format($valor, 2, ',', '.') : '';
                }
                ?>
                <div class="p-4 sm:p-7 min-h-[calc(100vh-145px)]">
                    <form id="form-valores-config" class="space-y-7">
                        <div class="px-2 py-1 mb-4">
                            <h2 class="text-lg font-semibold">Configuração de Valores de Depósito e Saque</h2>
                        </div>
                        <div class="grid grid-cols-1 lg:grid-cols-2 gap-7">
                        <div class="border border-black/10 dark:border-white/10 p-5 rounded-md">
                                <div class="mb-5">
                                    <p class="text-sm font-semibold">Ativar/Desativar Depósito em Dobro</p>
                                    <h2 class="text-xs font-italic">Ative o depósito em dobro para que o usuário receba o dobro do valor depositado.</h2>
                                </div>
                                <div>
                                    <div class="togglebutton inline-block">
                                        <label for="toggleD2" class="flex items-center cursor-pointer">
                                            <div class="relative">
                                                ...
                                                class="sr-only" <?php if ($config['deposito_dobro'] == 1)
                                                    echo 'checked'; ?>
                            ...
                            <div class="border border-black/10 dark:border-white/10 p-5 rounded-md">
                                <div class="mb-5">
                                    <p class="text-sm font-semibold">Depósito Mínimo</p>
                                    <h2 class="text-xs font-italic">Valor mínimo permitido para depósito.</h2>
                                </div>
                                <label class="mt-1 flex -space-x-px">
                                    <div class="flex items-center justify-center rounded-l-lg border border-black/10 dark:border-white/10 px-3.5">
                                        <span>R$</span>
                                    </div>
                                    <input id="deposito_min" name="deposito_min" class="form-input w-full border border-black/10 dark:border-white/10 bg-transparent px-3 py-2.5 placeholder:text-black/60 dark:placeholder:text-white/60 hover:z-10 hover:border-black dark:hover:border-white focus:z-10 focus:border-black dark:focus:border-white rounded-none" placeholder="0,00" type="text" value="<?php echo $config['deposito_min']; ?>">
                                    <div class="flex items-center justify-center rounded-r-lg border border-black/10 dark:border-white/10 px-3.5">
                                        <span>.00</span>
                                    </div>
                                </label>
                            </div>
                            <div class="border border-black/10 dark:border-white/10 p-5 rounded-md">
                                <div class="mb-5">
                                    <p class="text-sm font-semibold">Depósito Máximo</p>
                                    <h2 class="text-xs font-italic">Valor máximo permitido para depósito.</h2>
                                </div>
                                <label class="mt-1 flex -space-x-px">
                                    <div class="flex items-center justify-center rounded-l-lg border border-black/10 dark:border-white/10 px-3.5">
                                        <span>R$</span>
                                    </div>
                                    <input id="deposito_max" name="deposito_max" class="form-input w-full border border-black/10 dark:border-white/10 bg-transparent px-3 py-2.5 placeholder:text-black/60 dark:placeholder:text-white/60 hover:z-10 hover:border-black dark:hover:border-white focus:z-10 focus:border-black dark:focus:border-white rounded-none" placeholder="0,00" type="text" value="<?php echo $config['deposito_max']; ?>">
                                    <div class="flex items-center justify-center rounded-r-lg border border-black/10 dark:border-white/10 px-3.5">
                                        <span>.00</span>
                                    </div>
                                </label>
                            ...
                            <div class="border border-black/10 dark:border-white/10 p-5 rounded-md">
                                <div class="mb-5">
                                    <p class="text-sm font-semibold">Saque Mínimo</p>
                                    <h2 class="text-xs font-italic">Valor mínimo permitido para saque.</h2>
                                </div>
                                <label class="mt-1 flex -space-x-px">
                                    <div class="flex items-center justify-center rounded-l-lg border border-black/10 dark:border-white/10 px-3.5">
                                        <span>R$</span>
                                    </div>
                                    <input id="saque_min" name="saque_min" class="form-input w-full border border-black/10 dark:border-white/10 bg-transparent px-3 py-2.5 placeholder:text-black/60 dark:placeholder:text-white/60 hover:z-10 hover:border-black dark:hover:border-white focus:z-10 focus:border-black dark:focus:border-white rounded-none" placeholder="0,00" type="text" value="<?php echo $config['saque_min']; ?>">
                                    <div class="flex items-center justify-center rounded-r-lg border border-black/10 dark:border-white/10 px-3.5">
                                        <span>.00</span>
                                    </div>
                                </label>
                            ...
                            <div class="border border-black/10 dark:border-white/10 p-5 rounded-md">
                                <div class="mb-5">
                                    <p class="text-sm font-semibold">Saque Máximo</p>
                                    <h2 class="text-xs font-italic">Valor máximo permitido para saque.</h2>
                                </div>
                                <label class="mt-1 flex -space-x-px">
                                    <div class="flex items-center justify-center rounded-l-lg border border-black/10 dark:border-white/10 px-3.5">
                                        <span>R$</span>
                                    </div>
                                    <input id="saque_max" name="saque_max" class="form-input w-full border border-black/10 dark:border-white/10 bg-transparent px-3 py-2.5 placeholder:text-black/60 dark:placeholder:text-white/60 hover:z-10 hover:border-black dark:hover:border-white focus:z-10 focus:border-black dark:focus:border-white rounded-none" placeholder="0,00" type="text" value="<?php echo $config['saque_max']; ?>">
                                    <div class="flex items-center justify-center rounded-r-lg border border-black/10 dark:border-white/10 px-3.5">
                                        <span>.00</span>
                                    </div>
                                </label>
                            ...
                            <div class="border border-black/10 dark:border-white/10 p-5 rounded-md">
                                <div class="mb-5">
                                    <p class="text-sm font-semibold">Rollover</p>
                                    <h2 class="text-xs font-italic">Multiplicador de rollover exigido para saque. Exemplo: 2 = 2x o valor depositado.</h2>
                                </div>
                                <label class="mt-1 flex -space-x-px">
                                    <div class="flex items-center justify-center rounded-l-lg border border-black/10 dark:border-white/10 px-3.5">
                                        <span>X</span>
                                    </div>
                                    <input id="rollover" name="rollover" class="form-input w-full border border-black/10 dark:border-white/10 bg-transparent px-3 py-2.5 placeholder:text-black/60 dark:placeholder:text-white/60 hover:z-10 hover:border-black dark:hover:border-white focus:z-10 focus:border-black dark:focus:border-white rounded-none" placeholder="1.00" type="text" value="<?php echo $config['rollover']; ?>">
                                </label>
                            </div>
                        </div>
    ...
    <script>
        document.getElementById('form-valores-config')?.addEventListener('submit', function (e) {
            e.preventDefault();
            const form = this;
            const depMin = parseFloat(form.deposito_min.value.replace(',', '.'));
            const depMax = parseFloat(form.deposito_max.value.replace(',', '.'));
            const saqMin = parseFloat(form.saque_min.value.replace(',', '.'));
            const saqMax = parseFloat(form.saque_max.value.replace(',', '.'));
            const rollover = parseFloat(form.rollover.value.replace(',', '.'));

            if ([depMin, depMax, saqMin, saqMax].some(v => isNaN(v) || v < 0)) {
                showToast('Todos os valores devem ser numéricos e maiores ou iguais a zero.', 'error');
                return;
            }
            if (depMin > depMax) {
                showToast('O depósito mínimo não pode ser maior que o depósito máximo.', 'error');
                return;
            }
            if (saqMin > saqMax) {
                showToast('O saque mínimo não pode ser maior que o saque máximo.', 'error');
                return;
            }
            if (isNaN(rollover) || rollover < 1) {
                showToast('O rollover deve ser um número maior ou igual a 1.', 'error');
                return;
            }
            const fd = new FormData(form);
            fd.append('ajax_valores', 1);
            fetch('valores.php', { method: 'POST', body: fd })
                .then(r => r.json())
                .then(data => {
                    showToast(data.message, data.success ? 'success' : 'error');
                    if (data.success && data.config) {
                        // Atualiza valores na tela
                        document.getElementById('deposito_min').value = data.config.deposito_min;
                        document.getElementById('deposito_max').value = data.config.deposito_max;
                        document.getElementById('saque_min').value = data.config.saque_min;
                        document.getElementById('saque_max').value = data.config.saque_max;
                        document.getElementById('rollover').value = data.config.rollover;
                        
                        // Atualiza o toggle do depósito em dobro
                        const toggleCheckbox = document.getElementById('toggleD2');
                        if (toggleCheckbox) {
                            toggleCheckbox.checked = data.config.deposito_dobro == 1;
                        }
                    }
                });
        });
